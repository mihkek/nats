...
class TicketController extends Controller
{
    ...
    public function __construct(Ticket $tickets, Agent $agent)
    {
        ...
        $this->tickets = $tickets->with('user');
        $this->agent = $agent;
    }
    ...
    public function data(Request $request)
    {
        ...
        $collection
            ...
            ->select([
                ...
                DB::raw("CONCAT(users.first_name,' ',users.last_name,' ',users.middle_name) AS owner"),
                'ticketit.agent_id',
    ...
    public function dataMyHelpdest()
    {
        $userId = auth()->user()->id;
         $sql = "SELECT
                  `ticketit`.`id`,
                  ...
                  `ticketit`.`id` AS `agent`,
                  `ticketit_priorities`.`name` AS `priority`,
                  CONCAT(users.first_name, ' ', users.last_name, ' ', users.middle_name) AS owner,
                  `ticketit`.`agent_id`,
                  `ticketit_categories`.`name` AS `category`,
                ...
                FROM `ticketit`
                  INNER JOIN `users`
                    ON `users`.`id` = `ticketit`.`user_id`
                  INNER JOIN `ticketit_statuses`
                    ON `ticketit_statuses`.`id` = `ticketit`.`status_id`
                  INNER JOIN `ticketit_priorities`
                    ON `ticketit_priorities`.`id` = `ticketit`.`priority_id`
                  INNER JOIN `ticketit_categories`
                    ON `ticketit_categories`.`id` = `ticketit`.`category_id`
                WHERE `user_id` = " . $userId." ORDER BY `ticketit`.`updated_at` DESC";
        $data = DB::select($sql);
    ...
    public function store(Request $request)
    {
        ...
        return response(['status' => 1, 'msg' => $ticket], 200);
    }
    ...
    public function comment(Request $request)
    {
        $this->validate($request, [
            ...
            'helpdesc_id' => 'required',
        ]);

        $comment = new Comment();
        $comment->user_id = auth()->user()->id;
        $comment->content = $request->comment;
        $comment->html = "<p>" . $request->comment . "</p>";
        $comment->ticket_id = $request->helpdesc_id;

        return response(['status' => 1, 'msg' => $comment], 200);
    ...
    public function show(Request $request)
    {
        ...
        return response(['status' => 1, 'msg' => compact('ticket', 'status_lists', 'priority_lists', 'category_lists', 'agent_lists', 'comments',
            'close_perm', 'reopen_perm')], 200);
    }
  ...
  public function showApi(Request $request)
    {
        ...
        $comments = $ticket->comments()->with('user')->paginate($request->pagination);
    ...
    public function complete(Request $request)
    {
        ...
        return response(['status' => 401, 'msg' => 'У вас нет прав на это действие!'], 200);
    }
