      ...
      <v-layout row wrap>
        <v-flex md6 xs12>
          <v-card flat outlined class="mb-4">
            <v-card-title>
              <span v-if="auction.id > 0">Аукцион №{{auction.id}}</span>
              <span v-else>Новый аукцион</span>
            </v-card-title>
        ...
        <FieldAccess :model="prefix+'auction'" field="start_price" field_name="Стартовая цена" v-slot="{ accessData }">
              ...
              <!--<FieldAccess :model="prefix+'auction'" field="is_analog" field_name="Можно предлагать аналоги по ДВ" v-slot="{ accessData }" :style="item.is_analog == 1? 'display:block;':'display:none;'">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <FieldAccess :model="prefix+'auction'" field="title"
                    ...
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1"
                               @click.stop="editItem({ text: 'Наименование средства', val: 'exclude_analogs', type: 'multiselect2', options: v_select_title_options })">
              ...
              <!--<FieldAccess :model="prefix+'auction'" field="customer_warehouse_address" field_name="Адрес склада" v-slot="{ accessData }" :style="auction.delivery_condition == 2? 'display:block;':'display:none;'">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <!--<FieldAccess :model="prefix+'auction'" field="supplier_warehouse_address" field_name="Адрес склада поставщика" v-slot="{ accessData }" :style="auction.delivery_condition == 1 && (item.supplier_warehouse_address || item.status == 2) ? 'display:block;':'display:none;'">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <!--  <FieldAccess :model="prefix+'auction'" field="close" field_name="Завершить" v-slot="{ accessData }">
                  <v-divider class="my-0"></v-divider>
                  <v-list-item>
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        <v-btn
                          ...
                          class="rounded-btn"
                        >
                          <v-icon left size="20">mdi-flag-checkered</v-icon>Завершить аукцион
                        </v-btn>
                      </v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-icon>
                    ...
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        <v-btn
                          ...
                          class="rounded-btn"
                        >
                          <v-icon left size="20">mdi-cancel</v-icon>Отменить аукцион
                        </v-btn>
                      </v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-icon>
...
-->
    ...
    <v-dialog v-model="close_dialog" max-width="420">
      <v-card>
        ...
        <v-card-text>
          Вы действительно хотите досрочно окончить аукцион
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="cancel_dialog" max-width="420">
      <v-card color="#f6f6f6">
        ...
        <v-card-text>
          Вы действительно хотите отменить аукцион
        </v-card-text>
        <v-card-text>
            <v-select
...
//      link.download = filelink.substring(5)
  ...
  computed: {
    ...
    auction: {
      get() {
       if (this.$store.state.auction.item.type === 'rise') return this.$store.state.auction.item
      },
