  ...
  <v-card class="mb-3" flat outlined v-if="item.status > 0">
    <v-list>
      <template v-if="item.status == 1">
        <FieldAccess :model="item.type == 'rise' ? 'auction' : 'tender' " field="rates_price"
        ...
        <FieldAccess :model="item.type == 'rise' ? 'auction' : 'tender' " field="rates_confirm"
        ...
        <FieldAccess :model="item.type == 'rise' ? 'auction' : 'tender' " field="rates_cancel"
        ...
        <FieldAccess :model="item.type == 'rise' ? 'auction' : 'tender' " field="rates_add"
      ...
      <template v-if="item.rates.length > 0">
        <FieldAccess :model="item.type == 'rise' ? 'auction' : 'tender' " field="rates_history"
  ...
  methods: {
    ...
    setRateAnalog() {  
      /*if (this.exclude_analogs != 'underfined' && this.exclude_analogs != undefined && this.exclude_analogs != null) {
        this.vselect_titles = this.vselect_titles.filter(item => !this.item.exclude_analogs.includes(item))
      }*/
