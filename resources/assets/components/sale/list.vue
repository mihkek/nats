                    ...
                    <tr class="table-row font-weight-medium"  @click.stop="goTo(item)">
                      ...
                      <td>
                        <v-list-item class="px-0">
                          ...
                          <v-list-item-content v-else>
                            <v-list-item-title class="grey--text text--lighten-1 title" v-if="item.sale_rate != undefined">{{ formatPrice(item.sale_rate.price) }} ₽</v-list-item-title>
                            <v-list-item-title class="grey--text text--lighten-1 title" v-else>{{ formatPrice(item.start_price) }} ₽</v-list-item-title>
                          </v-list-item-content>
                        </v-list-item>
                      </td>
                      <td>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: 'Наименование / Регион отгрузки', value: 'title'},
...
//    console.log('own:'+this.own+' now:'+this.now)
  ...
  methods: {
    ...
    getItems() {
      ...
      /*console.log(params)*/
      this.$store.dispatch('GET_SALES', params).then(res => {
...
//	  console.log('filter updated');
  ...
  computed: {
    ...
    items: {
      get() {
        return this.$store.state.auction.sales
      }
    },
    auth_user: {
      get() {
