  ...
  <v-card class="mb-3" flat outlined v-if="item.status > 0">
    <v-list>
      <template v-if="item.status == 1">
        <FieldAccess :model="'auction'" field="rates_price"
          ...
          <v-list-item v-if="item.rate != undefined">
            <v-list-item-content>
              ...
              <!--<a href="#" class="blue--text font-weight-medium mb-2" style="text-decoration: underline;"
                 v-if="auth_user.id == item.user_id || auth_user.role_id == 1000 || item.rate.user.id == auth_user.id">{{
                  item.rate.user.company_name
                }}</a>-->
            </v-list-item-content>
...
<!-- item.completed == 1 &&  (auth_user.id == item.user_id || auth_user.role_id == 1000) -->
        <FieldAccess :model="'auction'" field="rates_confirm"
                     field_name="Принять текущее предложение" v-slot="{ accessData }"
                     v-if="item.completed == 0 && auth_user.role_id == 101">
        ...
        <FieldAccess :model="'auction'" field="rates_cancel"
        ...
        <!--<FieldAccess :model="'auction'" field="rates_add"
      ...
      <template v-if="item.rates.length > 0">
        <FieldAccess :model="'auction'" field="rates_history"
          ...
          <v-card flat color="transparent">
            ...
            <v-list subheader>
              ...
              <v-list-item two-line v-for="rate in item.rates" :key="rate.id"
                ...
                <v-list-item-icon v-else-if="auth_user.role_id== 101" class="v-list-q">
            ...
            <v-list-item>
              ...
              <v-list-item-content>
                <!--<p class="grey--text font-weight-regular mb-2 mt-0"
                   v-html="item.status == 1 ? 'Текущее <br/>предложение' : 'Итоговое <br/>предложение' "></p>-->
                <p class="grey--text font-weight-regular mb-2 mt-0"
                   v-html="'Итоговое <br/>предложение'"></p>  
                <v-list-item-title class="display-2 success--text mb-2 font-weight-medium">{{ formatPrice(item.start_price) }}
                  ₽/{{ item.unit }}
                </v-list-item-title>
...
export default {
  ...
  mounted() {
    ...
    //console.log('auth_user.role_id',auth_user.role_id);
  ...
  methods: {
    ...
    confirmRate() {
      ...
      this.$store.dispatch('CONFIRM_AUCTION_SALE_RATE', params).then(res => {
        this.$emit('refresh')
    ...
    /*setRateAnalog() {       
      ...
      if (this.exclude_analogs != 'underfined' && this.exclude_analogs != undefined && this.exclude_analogs != null) {
    ...
    addRate() {
      ...
      if (this.valid == true && this.newRate.value > 0) {
        ...
        // распродажи
        if (this.item.type=='sale') { 


          if (this.newRate.value != this.item.start_price) {
              this.errorText = 'Ваше ставка должна быть равной стартовой цене ' + this.formatPrice(this.item.start_price) + ''
              this.errorRate = true
              return false
          }
           ...
           /*if (this.item.price == null) {
            ...
            if (this.newRate.value > this.item.start_price) {
              this.errorText = 'Ваше ставка должна быть ниже стартовой цены ' + this.formatPrice(this.item.start_price) + ''
              this.errorRate = true
              return false
            }
        ...
        //аукционы
        /*if (this.item.type == 'rise' && this.newRate.value <= this.item.start_price) {
          this.errorText = 'Ваше ставка должна превышать ' + this.formatPrice(this.item.start_price) + ''
          this.errorRate = true
          return false
        }
