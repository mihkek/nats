<template>
  <v-app id="vue_block">
    <v-container fluid grid-list-xl class="px-0 py-0">
      ...
      <span>Начните вводить название препарата и выберете необходимый из предложенного</span>
                ...
                <v-list-item>
                  <v-list-item-content style="padding-top:0;">
                    <v-list-item-subtitle>Действующее вещество
      ...
      <span>В этом окне вы увидите действующее вещество в составе выбранного препарата</span>
        ...
        <FieldAccess :model="prefix+'auction'" field="start_price" field_name="Стартовая цена" v-slot="{ accessData }">
                ...
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Стартовая цена (₽) *
      <v-tooltip right>
      <template v-slot:activator="{ on, attrs }">
        <v-icon
        small
      ...
      <span>Максимальная сумма, которую получает продавец в самом начале распродажи</span>
    </v-tooltip>
   ...
   <FieldAccess :model="prefix+'auction'" field="start_price_step" field_name="Сколько шагов понижения цены" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Сколько шагов понижения цены *
      <v-tooltip right>
      <template v-slot:activator="{ on, attrs }">
        <v-icon
        small
      ...
      <span>Распродажа подразумевает снижение цены. Здесь вы можете указать, сколько раз вы готовы понизить "стартовую цену" за весь период распродажи
</span>
    </v-tooltip>

                    </v-list-item-subtitle>
                    <p class="my-0">{{ item.start_price_step || '0' }}</p>
                  </v-list-item-content>
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        ...
                        @click.stop="editItem({ text: 'Сколько шагов понижения цены', val: 'start_price_step',
                         type: 'select', options: start_price_steps })"
...
<!--1, 2, 3, 4, 5-->
   ...
   <FieldAccess :model="prefix+'auction'" field="start_price_percent" field_name="Какой шаг понижения цены в процентах" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Какой шаг понижения цены в процентах *
      <v-tooltip right>
      <template v-slot:activator="{ on, attrs }">
        <v-icon
        small
      ...
      <span>Укажите количество процентов, на которое уменьшится крайняя цена
</span>
    </v-tooltip>

                    </v-list-item-subtitle>
                    <p class="my-0">{{ item.start_price_percent || '0' }}</p>
                  </v-list-item-content>
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        ...
                        @click.stop="editItem({ text: 'Какой шаг понижения цены в процентах', val: 'start_price_percent',
                         type: 'select', options: start_price_percents })"
...
<!--5, 10, 15, 20, 25, 30-->
      ...
      <span>Укажите ваш объем препарата</span>
    </v-tooltip>
                  ...
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1"
                         @click.stop="editItem({ text: 'Объем', val: 'size', type: 'number' })"><v-icon>mdi-sync</v-icon></v-btn>
                      </template>
                      <span class="caption" v-html="__('buttons.edit_tooltip')"></span>
                    </v-tooltip>
                    <FieldAccessButton :accessData="accessData" />
      ...
      <span>Выберите необходимую единицу измерения (килограммы или литры)</span>
              ...
              <FieldAccess :model="prefix+'auction'" field="over_date" field_name="Дата окончания распродажи" v-slot="{ accessData }">
                ...
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Дата окончания распродажи *
      <v-tooltip right>
      <template v-slot:activator="{ on, attrs }">
        <v-icon
        small
      ...
      <span>Указав дату окончания распродажи, вы будете понимать, когда будет заключен договор на отгрузку</span>
    </v-tooltip>
      ...
      <span>Указав дату поставки, вы будете знать, когда отгружать препарат покупателю</span>
    </v-tooltip>
      ...
      <span>Выберите один из трех удобных способов отгрузки средства покупателю</span>
    </v-tooltip>
      ...
      <span>Укажите дополнительные пожелания к вашей распродаже</span>
    </v-tooltip>
                    </v-list-item-subtitle>
