    ...
    <v-flex xs12>
        ...
        <v-card color="transparent" flat class="my-3">
            <v-card-title class="pb-0 justify-center">
                <h1 class="my-0 last-title">Последние аукционы</h1>
                <v-flex class="text-center">
                    <a href="/admin/auction/add" class="v-btn v-btn--is-elevated v-btn--has-bg theme--light v-size--large success">
                        <span class="v-btn__content">Разместить аукцион</span>
                    </a>
                </v-flex>
            </v-card-title>
        </v-card>
		...
		Аукционы с заданными критериями не найдены
		</v-card>
        <v-data-table
          :headers="headers"
          :items="auctions"
            ...
            <v-container fluid grid-list-lg>
              <v-layout row wrap>
                <v-flex xs12 v-for="(item, i) in items" :key="i">
                  ...
                  <!-- КАРТОЧКА ТЕНДЕРА -->
                  <auction_card :item="item" @open="open(item)"/>
        ...
        <v-flex xs12 class="my-4">
          ...
          <v-flex class="text-center"><v-btn><a href="/auctions">Показать больше</a></v-btn></v-flex>

        </v-flex>
        <v-dialog v-model="dialog" scrollable max-width="500px">
            <auction_dialog @close="dialog = false" :item="selected" />
        </v-dialog>
    </v-flex>
</template>
...
import auction_dialog from './auction/auction_dialog'
  ...
  methods: {
    open(item) {
      ...
      if (item.auction.id !== undefined) {
        location.href = "/auctions/" + item.auction.id;
      }
    },
    getItems() {
      this.loading = true
