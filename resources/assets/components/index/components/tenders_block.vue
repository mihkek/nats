<template>
    <v-flex xs12>
        ...
        <v-card color="transparent" flat class="my-3">
            <v-card-title class="pb-0 justify-center">
                ...
                <v-flex class="text-center">
                    <a href="/admin/tender/add" class="v-btn v-btn--is-elevated v-btn--has-bg theme--light v-size--large success">
                        <span class="v-btn__content">Разместить тендер</span>
                    </a>
                </v-flex>
            </v-card-title>
        </v-card>
		...
		Тендеры с заданными критериями не найдены
		</v-card>
        <v-data-table
          :headers="headers"
          :items="auctions"
            ...
            <v-container fluid grid-list-lg>
              <v-layout row wrap>
                <v-flex xs12 v-for="(item, i) in items" :key="i">
                  ...
                  <!-- КАРТОЧКА ТЕНДЕРА -->
                  <tender_card :item="item" @open="open(item)"/>
        ...
        <v-flex xs12 class="my-4">
          ...
          <v-flex class="text-center"><v-btn ><a href="/tenders">Показать больше</a></v-btn></v-flex>

        </v-flex>
        <v-dialog v-model="dialog" scrollable max-width="500px">
            <tender_dialog @close="dialog = false" :item="selected" />
        </v-dialog>
    </v-flex>
</template>
...
import tender_card from './auction/tender_card'
import tender_dialog from './auction/tender_dialog'
  ...
  methods: {
    open(item) {
      ...
      if (item.auction.id !== undefined) {
        location.href = "/tenders/" + item.auction.id;
      }
      //console.log(item);
    },
    getItems() {
