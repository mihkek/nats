      ...
      <v-layout row wrap>
        <v-flex md6 xs12>
          <v-card flat outlined class="mb-4">
            <v-card-title>
              <span v-if="auction.id > 0">{{auction.type == 'rise' ? 'Аукцион' : 'Тендер' }} №{{auction.id}}</span>
              <span v-else>Новый {{this.type == 'rise' ? 'аукцион' : 'тендер' }}</span>
            ...
            <v-list subheader>
			  ...
			  <FieldAccess :model="prefix+'tender'" field="title" field_name="Наименование препарата" v-slot="{ accessData }">
...
//				if (this.confirmation) this.confirm(this.requisits_of)
  ...
  computed: {
    ...
    auction: {
      get() { 
        if (this.$store.state.auction.item.type === 'drop') return this.$store.state.auction.item
      },
