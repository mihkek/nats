    ...
    <v-main>
      <v-container fluid id="main" grid-list-xl class="px-0 py-0">
        ...
        <v-toolbar color="transparent" flat dense v-if="auth_user.role_id == 1000">
          <v-spacer></v-spacer>
          <v-btn color="warning" class="rounded-btn" @click.stop="getHistoryXls()" small>
...
export default {
  ...
  methods: {
    ...
    goTo(item) {
      let val = ''
      if (item.status == 1) {
        val = '/admin/tender/now/card/' + item.id
      } 
      location.href = val
    },
    ...
    getHistoryItems() {
      ...
      axios.post('/api/auction/get_user_list', params).then(res => {
        if (res.data !== undefined) {
          console.log(res.data)
          this.items = res.data.auctions
          this.setHistory()
          this.loading = false
        }
       })
