  ...
  <v-app id="vue_block">
    <v-container fluid grid-list-xl class="px-0 py-0">
        ...
        <v-flex md6 xs12>
          <v-card flat outlined class="mb-4">
            ...
            <v-list subheader>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="title"
                    ...
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1"
                               @click.stop="editItem({ text: 'Наименование средства', val: 'title', type: 'v-select', options: v_select_title_options })">
      ...
      <span>В этом окне вы увидите действующее вещество в составе выбранного препарата</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="is_analog"
      ...
      <span>Укажите, можно ли вам предлагать аналоги по действующему веществу препарата</span>
                  ...
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1"
                               @click.stop="editItem({ text: 'Можно аналоги', val: 'is_analog', type: 'select', options: [ { id: 1, title: 'Да' }, { id: 0, title: 'Нет' } ] })">
                          <v-icon>mdi-sync</v-icon>
                        </v-btn>
                      </template>
                      <span class="caption" v-html="__('buttons.edit_tooltip')"></span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="title"
                           field_name="Исключить аналоги" v-slot="{ accessData }" v-if="item.is_analog">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
      ...
      <span>Вы можете исключить аналоги по действующему веществу выбранного вами препарата</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="size"
      ...
      <span>Укажите необходимый объем поставки</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="unit"
      ...
      <span>Выберите необходимую единицу измерения (килограммы или литры)</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="over_date"
      ...
      <span>Указав дату окончания тендера, вы будете понимать, когда будет заключен договор на поставку</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="delivery_date"
      ...
      <span>Указав дату поставки, вы будете знать, когда вы получите свой товар</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' "
      ...
      <span>Выберите один из трех удобных способов поставки приобретенного товара</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="delivery_region"
      ...
      <span>Вы можете выбрать предоплатный или постоплатный способ расчета за приобретаемый товар</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="special_terms"
      ...
      <span>Укажите дополнительные пожелания к вашему тендеру</span>
              ...
              <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="docs"
      ...
      <span>Укажите дополнительные документы</span>
              ...
              <template
                ...
                <!--  <FieldAccess :model="auction.type == 'rise' ? prefix+'auction' : prefix+'tender' " field="close" field_name="Завершить" v-slot="{ accessData }">
                    <v-divider class="my-0"></v-divider>
                    <v-list-item>
                      <v-list-item-content class="text-center">
                        <v-list-item-subtitle class="text-center">
                    ...
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        ...
                        >
                          ...
                          Отменить {{ auction.type == 'rise' ? 'аукцион' : 'тендер' }}
                        </v-btn>
                      </v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-icon>
    ...
    <v-dialog v-model="close_dialog" max-width="420">
      <v-card>
        ...
        <v-card-text>
          Вы действительно хотите досрочно окончить {{ auction.type == 'rise' ? 'аукцион' : 'тендер' }}
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="cancel_dialog" max-width="420">
      <v-card>
        ...
        <v-card-text>
          Вы действительно хотите отменить {{ auction.type == 'rise' ? 'аукцион' : 'тендер' }}
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
