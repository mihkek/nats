        ...
        <v-card color="transparent" flat>
          <ul class="list-group pl-0 ml-0">
            ...
            <v-card flat style="border-right: 1px solid #ddd; border-left: 1px solid #ddd;">
              <v-card-title primary-title class="pb-4 pt-0"><v-icon class="mr-4">mdi-wallet</v-icon> <span v-html="__('billing.new_pay')"></span></v-card-title>
                  ...
                  <v-layout row wrap>
                    ...
                    <v-flex md8>
                      <v-text-field
                        ...
                        @click.stop="editTableItem( __('billing.customer_label'), 'user', customer_users, user_headers)"
                      >
                        <template v-slot:label>
                           <span v-html="__('billing.customer_label')"></span>
                        </template>
                      </v-text-field>
                    </v-flex>
                    <v-flex xs12>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('GET_CUSTOMER_USERS', { user_id: this.user_id })
  },
  methods: {
    submitSelect() {
    ...
    submit() {
      ...
      this.$store.dispatch('CREATE_USER_BILLING_PAY', data).then(res => {
        this.item = {}
      })
    },
  ...
  computed: {
    ...
    customer_users: {
      get() { return this.$store.state.configer_users.customer_users },
    },
  },
};
</script>
