...
export default {
  ...
  methods: {
    getItems() {
      this.$store.dispatch('GET_SUBSCRIPTIONS_HISTORY', { user_id: this.user_id }).then(res => { this.loading = false; })
  ...
  computed: {
    ...
    items: {
      get() { return this.$store.state.billing.subscriptions_history }
    },
  },
};
</script>
