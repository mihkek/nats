 <template>
  <v-card flat tile>
      <v-sheet height="64">
        <v-toolbar flat>
          <v-btn outlined class="mr-4" color="grey darken-2" @click="setToday" v-html="__('calendars.today_label')">{{__('calendars.today_label')}}</v-btn>
        ...
        >
          ...
          <template v-slot:event="{ event }">
            <v-card flat color="transparent" dark class="px-1">
              ...
              &mdash;
              <span :class="event.name == 'directory_person.busy' ? 'no_translate' : ''" :style="{textTransform: 'lowercase'}">{{event.name}}</span>
            </v-card>
          </template>
          ...
          <v-card flat max-width="360px">
            ...
            <v-card-actions>
              ...
              <v-btn color="primary" text :href="'/admin/orderer/now/card/'+selectedEvent.id" small>
                <span :class="__('buttons.follow_label') != 'orderer.follow_label' ? '' : 'empty'" v-html="__('buttons.follow_label')"></span> <v-icon right small>mdi-share</v-icon>
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-menu>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      busy_times: {
        get() { return this.$store.state.directory_people.busy_calendar_times },
        set(val) { this.$store.state.directory_people.busy_calendar_times = val },
      }
    },
    created() {
    ...
    methods: {
      ...
      getBusy() {
        this.$store.dispatch('GET_DIRECTORY_PERSON_BUSY_CALENDAR_TIMES', this.item).then(res => {
          this.busy_times.map(item => {
            this.events.push({
              name: __('directory_person.busy'),
