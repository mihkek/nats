          ...
          <v-card outlined class="rounded-card mt-1">
            ...
            <template v-slot:item="{ item }">
              <tr class="table-row" @click.stop="goTo('/admin/orderer/now/card/'+item.id)">
                ...
                <td class="ava d-none d-md-table-cell">
                  <v-list-item dense class="px-0" v-if="item.directory_firm != null">
                    <v-list-item-avatar>
                      <v-avatar size="50" color="transparent">
                        <img  v-if="item.directory_firm.avatar != null" :src="'/storage/avatars/50x50.'+item.directory_firm.avatar" alt="">
                        <img v-else src="/storage/avatars/50x50.nophoto.png" alt="">
                      </v-avatar>
                    </v-list-item-avatar>
                    <v-list-item-content><a class="font-weight-medium" :href="'/admin/directory/firms/'+item.directory_firm.id">{{item.directory_firm.full_name || ''}}</a></v-list-item-content>
                  </v-list-item>
                  <v-list-item dense class="px-0" v-if="item.additional_place != null">
                    <v-list-item-avatar>
                      <v-avatar size="50" color="transparent">
                        <img v-if="item.additional_place.avatar != null" :src="'/storage/avatars/additional_places/'+item.additional_place.avatar" alt="">
                        <img v-else src="/storage/avatars/50x50.nophoto.png" alt="">
                      </v-avatar>
                    </v-list-item-avatar>
                    <v-list-item-content><a class="font-weight-medium">{{item.additional_place.title || ''}}</a></v-list-item-content>
                  </v-list-item>
                </td>
                <td class="d-none d-md-table-cell"><v-rating dense size="14" :value="Number(item.customer_review_rate)" color="white" readonly></v-rating></td>
              </tr>
            </template>
            </v-data-table>
          </v-card>
...
export default {
  ...
  data: () => ({
    headers: [
      ...
      { text: __('orderer.directory_firm'), value: 'directory_firm.full_name', class: 'd-none d-md-table-cell' },
      { text: __('orderer.customer_review_rate'), value: 'customer_review_rate', class: 'd-none d-md-table-cell'  },
      /*{ text: __('orderer.price_label'), value: 'amount', class: 'd-none d-md-table-cell'  },*/
    ],
    search: null,
    expanded: [],
  }),
