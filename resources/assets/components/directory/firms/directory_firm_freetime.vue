            ...
            <ul class="list-group pl-0 ml-0 mb-0">
              <li class="list-group-item py-2" style="border-bottom: 0px;"></li>
                <v-list class="py-0" dense>
                  <FieldAccess model="directory_firm_freetime" field="add_new_note" :field_name="__('freetime.title')" v-slot="{ accessData }">
                    <v-list-item class="cardform-list-item pb-2" :class="accessData.canEdit ? 'pr-1' : ''">
                      <v-list-item-content class="inline-block" ><h4 class="font-weight-bold my-0" v-html="__('freetime.title')"></h4></v-list-item-content>
...
export default {
  ...
  methods: {
    ...
    newBusy() {
      this.selectedItem = {}
      this.selectedItem.directory_firm_id = this.item.id
      this.dialog = true
    },
    submit() {
      console.log(this.selectedItem)
      this.$store.dispatch('EDIT_DIRECTORY_FIRM_BUSY_TIME', this.selectedItem).then(res => {
        this.getItems()
      })
    },
    submitDelete() {
      this.$store.dispatch('DELETE_DIRECTORY_FIRM_BUSY_TIME', this.selectedItem).then(res => {
        this.getItems()
      })
    },
    getItems() {
      this.$store.dispatch('GET_DIRECTORY_FIRM_BUSY_TIMES', this.item).then(res => {
        this.dialog = false
  ...
  computed: {
    busy_times: {
      get() { return this.$store.state.directory_firms.busy_times },
      set(val) { this.$store.state.directory_firms.busy_times = val },
    }
  },
  directives: {  mask },
};
