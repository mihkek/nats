      ...
      <v-layout row wrap>
        <v-flex md6 xs12>
          <v-card flat outlined class="mb-4">
            <v-card-title>
              ...
              <div v-if="auth_user.role_id == 102">
                <v-chip
                  label
                  color="teal"
                  dark
            ...
            <v-list subheader>
			  ...
			  <FieldAccess :model="prefix+'tender'" field="title" field_name="Наименование действующего вещества" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Наименование препарата и действующего вещества</v-list-item-subtitle>
                    <p class="my-0" v-if="item.title !== item.active_material">{{ item.title || '-' }}</p>
                  ...
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1" @click.stop="editItem({ text: 'Наименование действующего вещества', val: 'title', type: 'v-select', options: dv_options })"><v-icon>mdi-sync</v-icon></v-btn>
                      </template>
              ...
              <!--<FieldAccess :model="prefix+'tender'" field="is_analog" field_name="Можно предлагать аналоги по ДВ" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Можно предлагать аналоги по ДВ</v-list-item-subtitle>
                    <p class="my-0" v-if="item.is_analog == 1">Да</p>
                    <p class="my-0" v-else-if="item.is_analog == 0">Нет</p>
                    <p class="my-0" v-else>-</p>
                  </v-list-item-content>
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1" @click.stop="editItem({ text: 'Можно аналоги', val: 'is_analog', type: 'select', options: [ { id: 1, title: 'Да' }, { id: 0, title: 'Нет' } ] })"><v-icon>mdi-sync</v-icon></v-btn>
                      </template>
                      <span class="caption" v-html="__('buttons.edit_tooltip')"></span>
                    </v-tooltip>
                    <FieldAccessButton :accessData="accessData" />
              ...
              <!--<FieldAccess :model="prefix+'tender'" field="title"
                           field_name="Исключить аналоги" v-slot="{ accessData }"  :style="item.is_analog == 1? 'display:block;':'display:none;'">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Исключить аналоги</v-list-item-subtitle>

                    <p class="my-0">
                      <template v-if="item.exclude_analogs" v-for="anlg in item.exclude_analogs">
                        {{ anlg + " " }}
                      </template>
                      <template v-else="">
                        -
              ...
              <FieldAccess :model="prefix+'tender'" field="size" field_name="Объем" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <FieldAccess :model="prefix+'tender'" field="unit" field_name="Единица измерения" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <FieldAccess :model="prefix+'tender'" field="over_date" field_name="Дата окончания тендера" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Дата окончания тендера</v-list-item-subtitle>
              ...
              <FieldAccess :model="prefix+'tender'" field="delivery_date" field_name="Дата поставки" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <FieldAccess :model="prefix+'tender'" field="delivery_condition" field_name="Условия поставки" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
			  ...
			  <FieldAccess :model="prefix+'tender'" field="delivery_region" field_name="Регион поставки" v-slot="{ accessData }" :style="auction.delivery_condition == 2 || auction.delivery_condition == 3? 'display:block;':'display:none;'">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                  ...
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1" @click.stop="editItem({ text: 'Регион поставки', val: 'delivery_region', type: 'r-select', options: delivery_regions })"><v-icon>mdi-sync</v-icon></v-btn>
                      </template>
                      <span class="caption" v-html="__('buttons.edit_tooltip')"></span>
                    </v-tooltip>
                    <FieldAccessButton :accessData="accessData" />
              ...
              <FieldAccess :model="prefix+'tender'" field="customer_warehouse_address" field_name="Адрес склада покупателя" v-slot="{ accessData }" :style="auction.delivery_condition == 2? 'display:block;':'display:none;'">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle>Адрес склада покупателя</v-list-item-subtitle>
              ...
              <FieldAccess :model="prefix+'tender'" field="supplier_warehouse_address" field_name="Адрес склада поставщика" v-slot="{ accessData }" :style="auction.delivery_condition == 1 && (item.supplier_warehouse_address || item.status == 2) ? 'display:block;':'display:none;'">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
                    ...
                    <p class="my-0" style="color:#ccc; font-weight:400;" v-else>Будет заполнено победившим поставщиком</p>
                  </v-list-item-content>
                  <v-list-item-icon v-if="accessData.canEdit">
                    <v-tooltip top v-if="!(auction.id > 0) || auth_user.role_id == 1000">
                      <template v-slot:activator="{ on }">
                        <v-btn color="red darken-1" icon x-small v-on="on" class="my-icon-btn mr-1" @click.stop="editItem({ text: 'Адрес склада', val: 'supplier_warehouse_address', type: 'text' })"><v-icon>mdi-sync</v-icon></v-btn>
                      </template>
                      <span class="caption" v-html="__('buttons.edit_tooltip')"></span>
                    </v-tooltip>
                    <FieldAccessButton :accessData="accessData" />
              ...
              <FieldAccess :model="prefix+'tender'" field="payment_condition" field_name="Условия оплаты" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <FieldAccess :model="prefix+'tender'" field="special_terms" field_name="Особые условия" v-slot="{ accessData }">
                <v-divider class="my-0"></v-divider>
                <v-list-item>
                  <v-list-item-content>
              ...
              <template v-if="!(auction.id > 0)">
                <FieldAccess :model="auction.type == 'rise' ? 'auction' : 'tender' "  field="create" field_name="Создать" v-slot="{ accessData }">
                  <v-divider class="my-0"></v-divider>
                  <v-list-item>
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        <v-btn
                          ...
                          :loading="loading"
                        >
                          <v-icon left size="20">mdi-plus</v-icon>Добавить {{auction.type == 'rise' ? 'аукцион' : 'тендер' }}
                        </v-btn>
              ...
              <!--  <FieldAccess :model="prefix+'tender'" field="close" field_name="Завершить" v-slot="{ accessData }">
                  <v-divider class="my-0"></v-divider>
                  <v-list-item>
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        <v-btn
                          depressed
                          color="orange"
                          @click.stop="close_dialog = !close_dialog"
                          text
                          class="rounded-btn"
                        >
                          <v-icon left size="20">mdi-flag-checkered</v-icon>Завершить {{auction.type == 'rise' ? 'аукцион' : 'тендер' }}
                        </v-btn>
                      </v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-icon>
                ...
                <FieldAccess :model="auction.type == 'rise' ? 'auction' : 'tender' "  field="cancel" field_name="Отменить" v-slot="{ accessData }">
                  <v-divider class="my-0"></v-divider>
                  <v-list-item>
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        <v-btn
                          ...
                          class="rounded-btn"
                        >
                          <v-icon left size="20">mdi-cancel</v-icon>Отменить {{auction.type == 'rise' ? 'аукцион' : 'тендер' }}
                        </v-btn>
                      </v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-icon>
                ...
                <div v-if="auth_user.role_id == 1000 && item.completed == 0">
                <FieldAccess :model="'tender'" field="close" field_name="Завершить досрочно" 
                v-slot="{ accessData }">
                  <v-divider class="my-0"></v-divider>
                  <v-list-item>
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        <v-btn
                          depressed
                          color="red"
                          @click.stop="closeTender"
                          text
                          class="rounded-btn"
                        >
                          <v-icon left size="20">mdi-cancel</v-icon>
                          Завершить досрочно тендер
                        </v-btn>
                      </v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-icon>
              ...
              <template v-if="item.id > 0 && item.complete_reason === null && item.user_id == auth_user.id">

                <FieldAccess :model="'tender'" field="complete" field_name="Завершить тендер" 
                v-slot="{ accessData }">
                  <v-divider class="my-0"></v-divider>
                  <v-list-item>
                    <v-list-item-content class="text-center">
                      <v-list-item-subtitle class="text-center">
                        ...
                        <v-btn
                            ...
                            color="success"
                            dark
                            @click.stop="completeTenderDialog"
                            class="rounded-btn"
                        >
              ...
              <template v-if="item.id > 0 && item.status == 0 && auth_user.role_id == 1000">
              
                <FieldAccess :model="'tender'" field="restore" field_name="Восстановить тендер" 
                v-slot="{ accessData }">
                  <v-divider class="my-0"></v-divider>
                  <v-list-item>
                    <v-list-item-content class="text-center">
              ...
              <template v-if="item.id > 0 && item.status == 2 && auth_user.role_id == 1000">
        ...
        <v-flex md6 xs12>
          ...
          <v-alert type="error" v-if="item.status == 0" class="mb-3">
            {{auction.type == 'rise' ? 'Аукцион' : 'Тендер' }} отменен организатором
          ...
          >
            <v-card flat color="transparent">
              <v-card-title class="title px-0 pt-0 success--text">
                {{auction.type == 'rise' ? 'Аукцион' : 'Тендер' }} завершен
              </v-card-title>
...
<!--
				<v-col class="shrink">
                  <v-btn color="success" small class="my-1 mr-2" @click.stop="uploadContract('customer')" v-if="auth_user.role_id == 1000 || item.user_id == auth_user.id">Загрузить подписанный договор{{ auth_user.role_id == 1000? ' покупателя':'' }}<v-icon right size="20">mdi-upload</v-icon></v-btn>
                  <v-btn color="success" small class="my-1 mr-2" @click.stop="uploadContract('supplier')" v-if="auth_user.role_id == 1000 || item.win_rate.user.id == auth_user.id">Загрузить подписанный договор{{ auth_user.role_id == 1000? ' поставщика':'' }}<v-icon right size="20">mdi-upload</v-icon></v-btn>
                </v-col>
-->
              </v-row>
				  <v-row
          ...
          <v-card class="mb-3" flat outlined v-if="item.user != false && (auth_user.role_id == 1000 || auth_user.role_id == 102 || (item.win_rate != undefined && item.win_rate.user.id == auth_user.id))">
            <v-list subheader>
            <v-subheader>Организатор аукциона <v-spacer></v-spacer><v-icon>mdi-city-variant</v-icon></v-subheader>
            ...
            <v-btn elevation="2" x-small class="my-1 success--text" v-if="auth_user.role_id === 102 || auth_user.role_id === 1000"
            :href="'/admin/tender/userlist/'+item.user.id"
            ><v-icon size="18" color="success">mdi-check</v-icon><span class="ml-1">Все тендеры покупателя</span></v-btn>
				  </v-list-item-subtitle>
          ...
          <template v-if="item.id > 0">
            ...
            <template v-if="item.id > 0 && item.doc_files !== undefined">
              <v-card class="mb-3" flat outlined>
                 <v-list>
                    <v-subheader>Вложенные файлы <v-spacer></v-spacer><v-icon>mdi-upload</v-icon></v-subheader>
                    <v-list-item v-for="(doc_file, index) in item.doc_files" :key="index">
                              <v-list-item-content>
                                <v-list-item-title>
                                  Файл 
                                  <v-btn elevation="2" x-small class="my-1 success--text" @click.stop="downloadDocFile(doc_file)"><v-icon size="18" color="success">mdi-check</v-icon><span class="ml-1">скачать</span></v-btn>
                                </v-list-item-title>                 
                              </v-list-item-content>
                    </v-list-item>
                    <v-list-item v-if="item.doc_files === null || item.doc_files.length === 0">
                      <v-list-item-content>
                                <v-list-item-title>Файлов нет</v-list-item-title>                           
                      </v-list-item-content>
                    </v-list-item>
                </v-list>
              </v-card>
    ...
    <v-dialog v-model="complete_dialog" max-width="420">
      <v-card color="#f6f6f6">
        <v-card-title>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="grey darken-3" text @click.stop="() => { complete_dialog = false, action_loading = false }">Отменить</v-btn>
          <v-btn color="grey darken-3" text @click.stop="completeTender" :loading="action_loading">Подтвердить</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    getDvOptions() {
      axios
          .get("/admin/auction/get_dv_options")
          .then(response => this.dv_options = response.data)
    },

    //191222
    addMyTender() {
       let params = {};
       params.auction_id = this.item_id; 
    ...
    closeTender() {
        ...
        axios.post('/admin/auction/close', params).then(res => {
          location.href = '/admin/tender/now/card/'+this.item_id;
        });
    },
    ...
    restoreTender() {
        let params = {};
        params.id = this.item_id;
        axios.post('/admin/auction/restore', params).then(res => {
          location.href = '/admin/tender/now/card/'+this.item_id;
        });
    },

    //160223
    completeTenderDialog() {
      this.complete_dialog = true;
    },

    //160223
    completeTender() {
      if (this.complete_reason !== '') {
      let params = {};
      params.id = this.item_id;
      params.complete_reason = this.complete_reason;
      axios.post('/admin/auction/complete', params).then(res => {
        //alert("Завершен");
        //this.complete_dialog = false;
        //this.item.complete_reason = this.complete_reason;
        alert(res.data.text);  
        location.href = '/admin/tender/now/card/'+this.item_id;
      });
      }
...
//      link.download = filelink.substring(5)
    ...
    //140223
    downloadDocFile(doc_file) {
          this.download_loading = true
      let params = {}
      params.doc_file = doc_file
      axios.post('/admin/auction/get_doc_file', params, {responseType: 'arraybuffer'}).then(res => {
        let blob = new Blob([res.data])
        let link = document.createElement('a')
        link.target = '_blank'
        link.href = window.URL.createObjectURL(blob)
    link.download = doc_file
        link.open = doc_file
        link.click()
      })
      .finally(() => (this.download_loading = false))
...
//				if (this.confirmation) this.confirm(this.requisits_of)
  ...
  computed: {
    ...
    auction: {
      get() { 
        if (this.$store.state.auction.item.type === 'drop' || this.$store.state.auction.item.type === 'dropdv') return this.$store.state.auction.item
