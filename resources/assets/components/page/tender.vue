...
<v-breadcrumbs
  ...
  <v-card v-if="item.auction">
    <v-card-title class="px-6 pt-6 mr-6" style="line-height: 1.2;">
      <h1>
        <span style="word-break:normal;">Тендер №{{item.auction.id}} {{ item.auction.title }} <span class="font-weight-regular">{{ item.auction.active_material }}</span>  </span>
      </h1>
    ...
    <v-card-text style="max-height: 75vh;" class="py-0 px-2">
      <v-container fluid grid-list-md class="py-0">
        ...
        <v-layout row wrap>
          ...
          <v-flex md4 v-if="item.auction.delivery_condition == 2 || item.auction.delivery_condition == 3">
            <v-list-item class="px-2">
              <v-list-item-content>
                <p class="mb-2 label text-left px-0" v-if="item.auction.delivery_region">Регион поставки:</p>
                <v-list-item-title>
        ...
        <v-layout row wrap>
            ...
            <v-list-item class="px-2">
              <v-list-item-content>
                ...
                <v-list-item-title>
                  ...
                  <span style="font-size: 24px; line-height: 32px; zoom: 1;" class="font-weight-bold grey--text" v-else>0 ₽</span>
                </v-list-item-title>
              </v-list-item-content>
            </v-list-item>
          </v-flex>
          <v-flex md8>
            <v-card-actions class="pt-6">
              <v-spacer></v-spacer>
              <v-btn color="success" large :href="'/admin/tender/now/card/'+item_id">Предложить цену</v-btn>
