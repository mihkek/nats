...
<v-breadcrumbs
  ...
  <v-card v-if="item.auction">
   <v-card-title v-if="item.auction.title.length > 0" class="px-6 pt-6 mr-6" style="line-height: 1.2;">
      <h1>
        <span style="word-break:normal;">Аукцион №{{item.auction.id}} {{ item.auction.title }} <span class="font-weight-regular">{{ item.auction.active_material }}</span>  </span>
      </h1>
        ...
        <v-layout row wrap>
          ...
          <v-flex md8>
            <v-card-actions class="pt-6">
              <v-spacer></v-spacer>
              <v-btn color="success" large :href="'/admin/auction/now/card/'+item.auction.id">Предложить цену</v-btn>
