...
@endif
			...
			<ul class="list-group">
				...
				<li class="list-group-item">
					Документ создал
					<span><a href="/admin/configer/users">{{ $document->user_name }}</a></span>
					<div style="clear:both;"></div>
				</li>
@if ($document->created)
				<li class="list-group-item">
...
@if ($document->filename)
				<li class="list-group-item">
					Файлы<br />
					<a href="/admin/documenter/download/{{ $document->id }}/pdf">
						{{ preg_replace("/\.docx$/", ".pdf", $document->filename) }}
					</a><br />
					<a href="/admin/documenter/download/{{ $document->id }}/doc">
						{{ $document->filename }}
					</a>
					<div style="clear:both;"></div>
				</li>
...
@if ($document->filename)
			...
			<a href="/admin/documenter/download/{{ $document->id }}/pdf" class="btn btn-lg btn-lgg btn-primary btnPause" style="width:100%;">
				<i class="icon-arrow-down"></i> &nbsp; СКАЧАТЬ PDF</a>
				<small><a href="/admin/configer/convertapi">Остаток лимитов на генерацию PDF-файлов</a></small>
			<br />
			<br />

			<a href="/admin/documenter/download/{{ $document->id }}/doc" class="btn btn-lg btn-lgg btn-success btnPause" style="width:100%;">
			...
			<br />
@endif
			<a href="/admin/templater/list" class="btn btn-lg btn-lgg btn-default" style="width:100%;">
				<i class="icon-action-undo"></i> &nbsp; ДРУГИЕ ДОКУМЕНТЫ</a>
		</div>
	...
	<div class="modal fade bs-modal-lg in" id="modal_email" tabindex="-1" role="basic" aria-hidden="true" style="display: none;">
		<div class="modal-dialog modal-lg">
			<form name="emailForm" id="emailForm" action="/admin/documenter/list" method="post">
...
{{--						<button type="submit" class="btn green">Отправить</button>--}}
				<a href="/admin/documenter/list" type="button" class="btn green">Отправить</a>
				</div>
			</div>
			</form>
		</div>
